...
p_dict = {"oxford": [p1, p2, p3, p4], "university": [p5, p6, p7], "residential": [p8, p9, p10], "business": [],
          "lgsvl": []}
p_dict_thresold = {"oxford": 25, "university": 25, "residential": 25, "business": 25, "lgsvl": 60}
...
def output_to_file(output, filename):
    if not os.path.exists('pickles'):
        os.mkdir('pickles')
    filename = os.path.join('pickles', filename)

    with open(filename, 'wb') as handle:
        pickle.dump(output, handle, protocol=pickle.DEFAULT_PROTOCOL)
    print("Done ", filename)
    ...
    for i in range(len(database_sets)):
        ...
        for j in range(len(test_sets)):
            ...
            for key in range(len(test_sets[j].keys())):
                ...
                index = tree.query_radius(coor, r=p_dict_thresold[output_name])
...
# For lgsvl District
# runs_folder = "lgsvl_new/"
# all_folders=sorted(os.listdir(os.path.join(BASE_DIR,base_path,runs_folder)))
# print(all_folders)
# construct_query_and_database_sets(base_path, runs_folder, all_folders, "/pointcloud_20m/", "pointcloud_locations_20m.csv", p_dict["lgsvl"], "lgsvl")
